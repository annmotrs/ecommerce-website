...
<body>
  ...
  <section class="orders container">
    ...
    <?php 
          try {
            ...
            //Выборка заказов со статусом "принято в работу", "доставляется", "в пункте выдачи"
            $select_orders = $conn->prepare('SELECT *, orders.id AS order_id FROM shop_db.orders 
                                              ...
                                              WHERE shop_db.orders.status = "принято в работу" OR shop_db.orders.status = "доставляется" OR shop_db.orders.status = "в пункте выдачи"
      ...
      ?>
        ...
        <div class="orders__action action" data-id="<?= $select_orders_array_row['order_id']; ?>">
          <div class="orders__status">
            <p class="orders__status-title"><label>Статус:</label></p>  
            <select class="select-status">
              <option value="принято в работу"<?= $select_orders_array_row['status'] === "принято в работу" ? " selected" : ""; ?>>принято в работу</option>
              <option value="доставляется"<?= $select_orders_array_row['status'] === "доставляется" ? " selected" : ""; ?>>доставляется</option>
              <option value="в пункте выдачи"<?= $select_orders_array_row['status'] === "в пункте выдачи" ? " selected" : ""; ?>>в пункте выдачи</option>
              <option value="выполнен"<?= $select_orders_array_row['status'] === "выполнен" ? " selected" : ""; ?>>выполнен</option>
            </select>               
          </div>     
  ...
  <script src="../js/updateStatusOrder.js"></script>
